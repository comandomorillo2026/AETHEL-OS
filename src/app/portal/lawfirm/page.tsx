...
function LawfirmContent() {
  const { lang, t } = useIndustryLanguage();
  const [billingCycle, setBillingCycle] = useState<'monthly' | 'annual'>('monthly');
  
  const common = t.common;
  const industry = t.lawfirm;
  const isEn = lang === 'en';

  const modules = [
    { icon: Scale, name: industry.modules[0].name, desc: industry.modules[0].desc },
    { icon: Users, name: industry.modules[1].name, desc: industry.modules[1].desc },
    { icon: Clock, name: industry.modules[2].name, desc: industry.modules[2].desc },
    { icon: FileText, name: industry.modules[3].name, desc: industry.modules[3].desc },
    { icon: Banknote, name: industry.modules[4].name, desc: industry.modules[4].desc },
    { icon: DollarSign, name: industry.modules[5].name, desc: industry.modules[5].desc },
    { icon: Calendar, name: industry.modules[6].name, desc: industry.modules[6].desc },
    { icon: Briefcase, name: industry.modules[7].name, desc: industry.modules[7].desc },
  ];
    ...
    <>
      ...
      <section className="relative z-10 py-20 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#C4A35A]/10 border border-[#C4A35A]/20 text-[#C4A35A] text-sm mb-6">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-[#EDE9FE] mb-6 leading-tight" style={{ fontFamily: 'var(--font-cormorant)' }}>
                {industry.heroTitle.split(' ').map((word, i) => 
                  i === 2 || i === 3 || i === 4 ? (
                    <span key={i} className="text-[#C4A35A]">{word} </span>
                  ) : (
            ...
            <div className="grid grid-cols-2 gap-4">
              {industry.stats.map((stat, i) => (
                <div key={i} className="p-6 rounded-xl bg-[rgba(108,63,206,0.05)] border border-[rgba(167,139,250,0.1)]">
                  <p className="text-3xl font-bold text-[#C4A35A]">{stat.value}</p>
      ...
      <section className="relative z-10 py-16 px-4 bg-[rgba(108,63,206,0.03)]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {modules.map((mod, i) => (
              <div key={i} className="p-4 rounded-xl bg-[rgba(108,63,206,0.05)] border border-[rgba(167,139,250,0.1)] hover:border-[#C4A35A]/30 transition-all">
                <mod.icon className="w-8 h-8 text-[#C4A35A] mb-3" />
      ...
      <section className="relative z-10 py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12">
              ...
              <ul className="space-y-4">
                {industry.benefits.map((benefit, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <CheckCircle className="w-5 h-5 text-[#C4A35A] mt-0.5 flex-shrink-0" />
            ...
            {/* Who it's for */}
            <div className="p-8 rounded-2xl bg-gradient-to-br from-[rgba(196,163,90,0.1)] to-[rgba(108,63,206,0.05)] border border-[rgba(167,139,250,0.1)]">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg transition-all ${
                  billingCycle === 'monthly' 
                    ? 'bg-[#C4A35A]/20 text-[#C4A35A] border border-[#C4A35A]/30' 
              ...
              return (
                <div 
                  ...
                  className={`p-6 rounded-2xl border transition-all ${
                    plan.popular 
                      ? 'bg-[rgba(196,163,90,0.05)] border-[#C4A35A]/30 scale-105' 
                      : 'bg-[rgba(108,63,206,0.05)] border-[rgba(167,139,250,0.1)]'
                  }`}
                >
                  {plan.popular && (
                    <div className="text-center mb-4">
                      <span className="px-3 py-1 rounded-full bg-[#C4A35A] text-[#050410] text-xs font-bold">
                        {common.popular}
                      </span>
                    </div>
                  )}
                  ...
                  <ul className="space-y-3 mb-6">
                    {plan.features.map((feature, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-[#EDE9FE]">
                        <CheckCircle className="w-4 h-4 text-[#C4A35A] mt-0.5 flex-shrink-0" />
                        {feature}
                      </li>
                    ))}
                  </ul>
                  ...
                  <a 
                    ...
                    className={`block w-full py-3 rounded-lg text-center font-medium transition-all ${
                      plan.popular 
                        ? 'bg-gradient-to-r from-[#C4A35A] to-[#8B7355] text-white hover:opacity-90' 
      ...
      <section className="relative z-10 py-16 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <div className="p-8 rounded-3xl bg-gradient-to-r from-[rgba(196,163,90,0.1)] to-[rgba(108,63,206,0.1)] border border-[rgba(167,139,250,0.2)]">
            <h2 className="text-2xl font-bold text-[#EDE9FE] mb-4">
              {common.readyToTransform} {isEn ? 'law firm?' : 'bufete?'}
            </h2>
            <p className="text-[#9D7BEA] mb-6">
              {isEn ? 'Join over 30 law firms in the Caribbean already using NexusOS' : 'Únete a más de 30 bufetes del Caribe que ya usan NexusOS'}
            </p>
            <a 
              href="/portal#apply"
...
// Main page component
export default function LawfirmPortalPage() {
  const screenshots = [
    { src: '/screenshots/lawfirm/dashboard.png', alt: 'Dashboard de Bufete' },
    { src: '/screenshots/lawfirm/cases.png', alt: 'Gestión de Casos' },
    { src: '/screenshots/lawfirm/time-tracking.png', alt: 'Seguimiento de Tiempo' },
  ];

  return (
    <IndustryPageLayout 
      industryKey="lawfirm"
      industryIcon={<Scale className="w-5 h-5 text-[#C4A35A]" />}
      industryColor="#C4A35A"
