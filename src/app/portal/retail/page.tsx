...
function RetailContent() {
  ...
  const industry = t.retail;
  const isEn = lang === 'en';

  const modules = [
    { icon: Store, name: industry.modules[0].name, desc: industry.modules[0].desc },
    { icon: Package, name: industry.modules[1].name, desc: industry.modules[1].desc },
    { icon: Users, name: industry.modules[2].name, desc: industry.modules[2].desc },
    { icon: Building2, name: industry.modules[3].name, desc: industry.modules[3].desc },
    { icon: Calculator, name: industry.modules[4].name, desc: industry.modules[4].desc },
    { icon: BarChart3, name: industry.modules[5].name, desc: industry.modules[5].desc },
    { icon: Truck, name: industry.modules[6].name, desc: industry.modules[6].desc },
  ];
    ...
    <>
      ...
      <section className="relative z-10 py-20 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#3B82F6]/10 border border-[#3B82F6]/20 text-[#3B82F6] text-sm mb-6">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-[#EDE9FE] mb-6 leading-tight" style={{ fontFamily: 'var(--font-cormorant)' }}>
                {industry.heroTitle.split(' ').map((word, i) => 
                  i === 2 || i === 3 ? (
                    <span key={i} className="text-[#3B82F6]">{word} </span>
                  ) : (
      ...
      <section className="relative z-10 py-16 px-4 bg-[rgba(108,63,206,0.03)]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {modules.map((mod, i) => (
              <div key={i} className="p-4 rounded-xl bg-[rgba(108,63,206,0.05)] border border-[rgba(167,139,250,0.1)] hover:border-[#3B82F6]/30 transition-all">
                <mod.icon className="w-8 h-8 text-[#3B82F6] mb-3" />
      ...
      <section className="relative z-10 py-16 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            {/* Who it's for */}
            <div className="p-8 rounded-2xl bg-gradient-to-br from-[rgba(59,130,246,0.1)] to-[rgba(108,63,206,0.05)] border border-[rgba(167,139,250,0.1)]">
              ...
              return (
                <div 
                  ...
                  className={`p-6 rounded-2xl border transition-all ${
                    plan.popular 
                      ? 'bg-[rgba(59,130,246,0.05)] border-[#3B82F6]/30 scale-105' 
      ...
      <section className="relative z-10 py-16 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <div className="p-8 rounded-3xl bg-gradient-to-r from-[rgba(59,130,246,0.1)] to-[rgba(108,63,206,0.1)] border border-[rgba(167,139,250,0.2)]">
            <h2 className="text-2xl font-bold text-[#EDE9FE] mb-4">
              {common.readyToTransform} {isEn ? 'store?' : 'tienda?'}
            </h2>
            <p className="text-[#9D7BEA] mb-6">
              {isEn ? 'Join over 40 retail stores in the Caribbean already using NexusOS' : 'Únete a más de 40 tiendas del Caribe que ya usan NexusOS'}
            </p>
            <a 
              href="/portal#apply"
...
// Main page component
export default function RetailPortalPage() {
  const screenshots = [
    { src: '/screenshots/retail/dashboard.png', alt: 'Dashboard de Retail' },
    { src: '/screenshots/retail/pos.png', alt: 'Punto de Venta' },
    { src: '/screenshots/retail/inventory.png', alt: 'Gestión de Inventario' },
  ];

  return (
    <IndustryPageLayout 
      industryKey="retail"
      industryIcon={<ShoppingBag className="w-5 h-5 text-[#3B82F6]" />}
